...
for s=1:sens
    ...
    [output_prev, output_cascade, output_cascade_PWID, output_disease, output_cases,output_ost,output_nsp, output_diagnoses] = ...
        calibrate_optim_par(20, 4);
    save('C:\Users\Nick\Desktop\Matlab Sims\Tanzania\calibration')
    ...
    target_late=-0.5; alpha = alpha_DAA; treat=treat_projected;
    ...
    if strcmp(scenario,'current') == 1
        for j = 1:length(elim_years)
            ...
            for i = 1:length(treat_range)
                ...
                incred(i,j) = (inc2015 - sum(sum(sum(ycomb5_noage(find(TT2_treat5>=elim_years(j),1):find(TT2_treat5>=elim_years(j)+1,1)-1,:,:,27)))))/ inc2015 ;%...
    ...
    for i = 1:length(range_test)
        for j = 1:length(range_followup)
            ...
            for year_elim = 1:Run-2
                incred_test(i,year_elim) = (inc2015 - sum(sum(sum(ycomb3_noage(find(TT2_treat3>=Tin+year_elim,1):find(TT2_treat3>=Tin+year_elim+1,1)-1,:,:,27)))))/...
                    (TT2_treat3(find(TT2_treat3>=Tin+year_elim+1,1)-1)-TT2_treat3(find(TT2_treat3>=Tin+year_elim,1))) / inc2015;
            end
