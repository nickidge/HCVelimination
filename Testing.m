...
for s=1:sens
    ...
    [output_prev, output_cascade, output_cascade_PWID, output_disease, output_cases] = calibrate_optim_par(4, 12);
    save('C:\Users\Nick\Desktop\Matlab Sims\Testing\sample')
    %load('C:\Users\Nick\Desktop\Matlab Sims\Testing\baseline_calibration')
    infect_base=infect;
    ...
    %reshape(sum(sum(sum(sum(sum(y2_treat(:,1,:,:,:,:,:,1:20),3),4),6),7),8),size(y2_treat,1),num_intervention)
    ...
    if strcmp(scenario,'current') == 1
        for j = 1:length(elim_years)
            treat_range = 2000:2000:55000; % Estimate treatments required
