global filename scenario sens target_late Tin Run start_year num_pops num_cascade num_age num_intervention num_engagement num_region dt ... %meta parameters
    ...
    harm_reduction_coverage nsp_coverage ost_coverage prop_test diagnosed_risk_reduction output_cases UHC
...
for s=1:sens
    ...
    filename=strcat(drive,":\Users\",user,"\Desktop\Matlab Sims\Tanzania\sens_annualtest");
 ...
 %% Run model in prior to 2017
    ...
    range_test = [0, 1]*2; % divided by 2 to assume that infection happens midway between tests
    range_followup = [0.74];
...
%                         sum(sum(sum(ycomb6_noage(find(TT2_treat6>=t,1),:,:,[6,12:20]))));
    ...
    progression(1,2,2,1) = 50; progression(2,2,2,1) = 50; progression(3,2,2,1) = 50; % perfect "RNA" followup
    range_followup = [0.74];
    range_test = [0,1]; % divided by 2 to assume that infection happens midway between tests
    range_diagnosed_risk_reduction = [0:0.1:0.1]; % risk reduction when diagnosed
    prop_test_range = [0.9];
    ...
    ICER = [[0,(summary_test(2,2,1,1,s) - summary_HR(1,1,s))/(summary_HR(1,2,s) -summary_test(2,2,1,2,s)), (summary_serum(2,2,1,1,s) - summary_HR(1,1,s))/(summary_HR(1,2,s) -summary_serum(2,2,1,2,s)),...
