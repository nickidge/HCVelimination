...
global filename scenario sens target_late Tin Run start_year num_pops num_cascade num_age num_intervention num_engagement num_region  dt ... %meta parameters
    ...
    p_complete total_PWID PWID0 age_mix prev0 harm_reduction r_inc_up followup prop_test ...%PWID parameters
...
target_late = 1;
alpha = alpha_DAA;
range_test = [-1, 0.5, 1, 2, 4]*2; % divided by 2 to assume that infection happens midway between tests
range_followup = [-1,0.5:0.1:1];
prop_test_range = [0.6,0.8,1];
treat = [150000,000,2000];
    ...
    for j = 1:length(range_followup)
        for k =1:length(prop_test_range)
            ...
            if range_followup(j) > 0
                ...
                progression(1,2,2,1) = (followup)/0.25 + temp_rate1 * (1-followup);
                progression(2,2,2,1) = (followup)/0.25 + temp_rate2 * (1-followup);
            ...
            for year_elim = 1:Run-2
                incred_test(i,year_elim,j,k) = (inc2015 - sum(sum(sum(ycomb3_noage(find(TT2_treat3>=Tin+year_elim,1):find(TT2_treat3>=Tin+year_elim+1,1)-1,:,:,27)))))/ inc2015;%...
                % (TT2_treat3(find(TT2_treat3>=Tin+year_elim+1,1))-TT2_treat3(find(TT2_treat3>=Tin+year_elim,1))) / inc2015;
            end
            if incred_test(i,end,j,k) >= 0.8
                year_elim_test(i,j,k) = 1950+Tin+find(incred_test(i,:,j,k)>=0.8,1);
            else
                year_elim_test(i,j,k) = -1;
            end
        end
    end
end
...
for j = 1:length(prop_test_range)
    scenario = 'current';
    prop_test = prop_test_range(j);
    progression = progression_base;
    progression(1,5,2,1) = 1/(30/365); progression(2,5,2,1) = 1/(30/365); progression(3,5,2,1) = 1/(30/365);
    progression(1,6,2,1) = 1/(30/365); progression(2,6,2,1) = 1/(30/365); progression(3,6,2,1) = 1/(30/365);
    progression(1,3,2,1) = 50; progression(2,3,2,1) = 50; progression(3,3,2,1) = 50; % remove genotype
    ...
    RNAtesting_range = [-1,1/2,1,2,4]*2; % divide by two to represent being infected midway between tests
    for i = 1:length(RNAtesting_range)
        if RNAtesting_range(i) > 0
            progression(2,2,2,1) = 50; progression(1,2,2,1) = 50; % get rid of second test requirement
            progression(1,1,2,1) = range_test(i);%*0.9 + 0.1*temp_rate;
            progression(2,1,2,1) = range_test(i);%*0.9 + 0.1*temp_rate0;
            followup = 1;
...
%% Plots
a0 = 100*reshape(incred_test(:,end,2:end,:),length(range_test),length(range_followup)-1,length(prop_test_range));
for i = 1:length(a0(:,1))
    for j = 2:length(a0(1,:))
        a(i,j) = max(0, min(a0(i,j)-a0(i,1), a0(i,j) - a0(i,j-1)));
    end
end
a1 = 100*reshape(incred_RNA(:,end,:),length(RNAtesting_range),length(prop_test_range));
for i = 1:length(a1(:,1))
    a2(i,1) = a1(i,1);
    for j = 2:length(a1(1,:))
        a2(i,j) = a1(i,j) - sum(a2(i,1:j-1));
    end
end
a3 = 0*a0(:,:,2);
a3(:,1) = a0(:,1,2);
a3(:,2) = a0(:,2,2) - a0(:,1,2);
a3(:,3) = a0(:,3,2) - a0(:,2,2);
a3(:,4) = max(0, a0(:,4,2) - a0(:,3,2));
a3(:,5) = max(0, a0(:,5,2) - a0(:,4,2));
a3(:,6) = max(0, a0(:,6,2) - a0(:,5,2));
...
figure(3)
subplot(1,2,1)
b = bar(a4(2:end,:),'stacked'); grid on;
...
ylim([0,100]); ylabel('Estimated incidence reduction in 2030 (%)');
legend('60% testing coverage', '80% testing coverage','100% testing coverage','location','southeast');
