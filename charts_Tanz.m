...
casc = zeros(3,4);
casc(1,1) = sum(sum(sum(ycomb_noage(find(TT2>=analysis_year-1950,1),1:3,1:10,6:20)))); % PLHCV %./sum(sum(sum(ycomb2_noage(find(TT2_treat>=Tin,1),1:3,1:10,6:20))));
casc(1,2) = 0;%sum(sum(sum(ycomb_noage(find(TT2>=analysis_year-1950,1),1:3,2:10,6:20))));% Ab diagnosed./sum(sum(sum(ycomb2_noage(find(TT2_treat>=Tin,1),1:3,1:10,6:20))));
casc(1,3) = 0;%sum(sum(sum(ycomb_noage(find(TT2>=analysis_year-1950,1),1:3,3:10,6:20))));%RNA diagnosed./sum(sum(sum(ycomb2_noage(find(TT2_treat>=Tin,1),1:3,1:10,6:20))));
casc(1,4) = 0;%PLHCV2016-casc(1,1)-sum(death_year(1,1:(analysis_year-1950-Tin))); %cured; %cured
casc(2,1) = sum(sum(sum(ycomb5_noage(find(TT2_treat>=analysis_year-1950,1),1:3,1:10,6:20)))); % PLHCV 
casc(2,2) = sum(sum(sum(ycomb5_noage(find(TT2_treat>=analysis_year-1950,1),1:3,2:10,6:20))));% Ab diagnosed
casc(2,3) = sum(sum(sum(ycomb5_noage(find(TT2_treat>=analysis_year-1950,1),1:3,3:10,6:20))));%RNA diagnosed
casc(2,4) = PLHCV2016-casc(2,1)-sum(death_year(2,1:(analysis_year-1950-Tin))); %cured
casc(3,1) = sum(sum(sum(ycomb6_noage(find(TT2_treat5>=analysis_year-1950,1),1:3,1:10,6:20)))); % PLHCV 
casc(3,2) = 0;%sum(sum(sum(ycomb6_noage(find(TT2_treat5>=analysis_year-1950,1),1:3,2:10,6:20))));% Ab diagnosed
casc(3,3) = sum(sum(sum(ycomb6_noage(find(TT2_treat5>=analysis_year-1950,1),1:3,3:10,6:20))));%RNA diagnosed
casc(3,4) = PLHCV2016-casc(3,1)-sum(death_year(5,1:(analysis_year-1950-Tin))); %cured
casc(4,1) = sum(sum(sum(ycomb3_noage(find(TT2_treat6>=analysis_year-1950,1),1:3,1:10,6:20)))); % PLHCV 
casc(4,2) = 0;%sum(sum(sum(ycomb3_noage(find(TT2_treat6>=analysis_year-1950,1),1:3,2:10,6:20))));% Ab diagnosed
casc(4,3) = sum(sum(sum(ycomb3_noage(find(TT2_treat6>=analysis_year-1950,1),1:3,3:10,6:20))));%RNA diagnosed
casc(4,4) = PLHCV2016-casc(4,1)-sum(death_year(6,1:(analysis_year-1950-Tin))); %cured
mod = casc/PLHCV2016;%./repmat(casc(:,1),1,4);
...
h2 = plot(prev_testing(:,1)', 'color', CML(1,:), 'linewidth',2);
d1 = scatter([5;8],[21;23],100,'filled','d','k');
%legend([d1,h(1),h2,h(3:end)],'Data','0% coverage','6% coverage (estimated current)','10% coverage','20%','30%','40%','50% coverage');
set(gca, 'Ylim',[0,40],'YTick',0:10:90,'YTickLabel',0:10:90, 'Xlim',[3,23],'XTick',3:5:23,'XTickLabel',2010:5:2030);
ylabel('HCV prevalence among PWID (%)');
...
h2_inc = plot(inc_testing(:,1), 'color', CML(1,:), 'linewidth',2);
target = plot(0.2*repmat(inc_HR(5,1),1,30),':k', 'linewidth',2); text(3.5,0.2*inc_HR(5,1)-150,'WHO 2030 target','fontsize',10);
