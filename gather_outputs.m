...
TT_=TT(t_val(1):t_val(2))-Tin;
[cost,qaly,life_exp,tr]=Costs_age(y2_noage(1:t_val(2)-t_val(1)+1,:,:,:),TT_);
tr_(1) = sum(sum(sum(y2_noage(1:t_val(3)-t_val(1)+1,1,:,[23,25])))); % Total treatments for PWID
tr_(2) = sum(sum(sum(y2_noage(1:t_val(3)-t_val(1)+1,2,:,[23,25])))); % Total treatments for former PWID
tr_(3) = sum(sum(sum(y2_noage(1:t_val(3)-t_val(1)+1,3,:,[23,25])))); % Total treatments for other
tr_(4) = sum(sum(sum(sum(y2_noage(1:t_val(3)-t_val(1)+1,:,:,[23,25]))))); % Total treatments
