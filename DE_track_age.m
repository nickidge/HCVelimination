...
global mu_PWID mu_former exit_IDU r_relapse delta alpha p_complete omega infect total_PWID PWID0 dem dt...
    ...
    ost_enrollment ost_duration nsp_enrollment nsp_duration RNAtesting harm_reduction_coverage ost_coverage nsp_coverage diagnosed_risk_reduction output_cases
...
%ODE subfunction
    for t = 2:length(tvec)
        ...
        if TT > start_year
            ...
            for j = 1:num_region
                for i =1:num_pops
                    ...
                    if sum(sum(sum(sum(Y(i,:,1,:,:,j,1:20))))) > 0 && sum(sum(sum(sum(sum(Y(i,:,2:end,:,:,j,1:20)))))) > 0
                        age_mix = sum(sum(sum(sum(sum(Y(p,:,1,:,:,:,1:20),2),4),5),6),7)/ sum(sum(sum(sum(sum(sum(Y(p,:,:,:,:,:,1:20),2),3),4),5),6),7);
                    ...
                    for a=2:num_age-1
                        if sum(sum(sum(sum(sum(Y(i,:,a,:,:,:,1:20)))))) > 0
                            age_mix = sum(sum(sum(sum(sum(Y(p,:,a,:,:,:,1:20),2),4),5),6),7)/ sum(sum(sum(sum(sum(sum(Y(p,:,:,:,:,:,1:20),2),3),4),5),6),7);
                    ...
                    if sum(sum(sum(Y(i,:,num_age,1:20)))) > 0
                        age_mix = sum(sum(sum(sum(sum(Y(p,:,num_age,:,:,:,1:20),2),4),5),6),7)/ sum(sum(sum(sum(sum(sum(Y(p,:,:,:,:,:,1:20),2),3),4),5),6),7);
            ...
            if TT>66
                if treat(1)<0
                    f(:,[6,8],:,:,:,:,12:20) = - treat(1) * Y(:,[6,8],:,:,:,:,12:20);
                else
                    trea=treat(1);
            ...
            %% New arrivals
            IDUs=PWID0;
            if TT<60 % TT = 50 is 2000
                IDUs = (TT-start_year)*(total_PWID-PWID0)/(60-start_year) + PWID0;
%             elseif TT>=50 && TT<55
%                 IDUs = total_PWID - (TT-50)*(total_PWID -total_PWID)/5;
            ...
            mu_arrivals = max(IDUs-sum(sum(sum(sum(sum(sum(sum(Y([1],:,:,:,:,:,1:20))))))))...
                +sum(sum(sum(sum(sum(sum(sum(-ydot3([1],:,:,:,:,:,1:20))))))))+sum(sum(sum(sum(sum(sum(sum(-ydot4([1],:,:,:,:,:,1:20)))))))),...
                0);
            ...
            elseif TT>=65 import_infections=imp9*max((70-TT)/5,0) + min((TT-65)/5,1)*imp9 * sum(sum(sum(sum(sum(sum(sum(Y(:,:,:,:,:,:,6:20))))))))/ output_cases(1) ; end
            ...
            ydot6(3,1,1,1,2,1,27)=1*import_infections;
...
% %             
            hr_scale1 = (nsp_coverage*sum(Y(1,:,:,:,:,:,1:20),4) - sum(Y(1,:,:,[2],:,:,1:20),4))./sum(Y(1,:,:,[1],:,:,1:20),4); %change in number of people needed, as a percentage of other compartments
            hr_scale2 = (ost_coverage*sum(Y(1,:,:,:,:,:,1:20),4) - sum(Y(1,:,:,[3],:,:,1:20),4))./sum(Y(1,:,:,[1],:,:,1:20),4); %change in number of people needed, as a percentage of other compartments
            hr_scale1(isnan(hr_scale1))=0; hr_scale2(isnan(hr_scale2))=0;
            ydot11(1,:,:,1,:,:,1:20) = ydot11(1,:,:,1,:,:,1:20) + max(-1/dt,min(1/dt, -hr_scale1-hr_scale2+1/ost_duration)).*Y(1,:,:,1,:,:,1:20);
            ydot11(1,:,:,2,:,:,1:20) = ydot11(1,:,:,2,:,:,1:20) + max(-1/dt,min(1/dt, hr_scale1)).*Y(1,:,:,1,:,:,1:20);
            ydot11(1,:,:,3,:,:,1:20) = ydot11(1,:,:,3,:,:,1:20) + max(-1/dt,min(1/dt, hr_scale2-1/ost_duration)).*Y(1,:,:,1,:,:,1:20);
...
%                 .*sum(Y(1,:,:,[1,3,4],:,:,1:20),4);
%             ydot11(1,:,:,3,:,:,1:20) = ydot11(1,:,:,3,:,:,1:20) + ...
%                 min(1/dt, ((1-nsp_coverage)*ost_coverage*sum(Y(1,:,:,:,:,:,1:20),4) - (Y(1,:,:,3,:,:,1:20)+ydot11(1,:,:,3,:,:,1:20)))./sum(Y(1,:,:,:,:,:,1:20),4))...
%                 .*sum(Y(1,:,:,[1,2,4],:,:,1:20),4);
%             ydot11(1,:,:,4,:,:,1:20) = ydot11(1,:,:,4,:,:,1:20) + ...
%                 min(1/dt, (nsp_coverage*ost_coverage*sum(Y(1,:,:,:,:,:,1:20),4) - (Y(1,:,:,4,:,:,1:20)+ydot11(1,:,:,4,:,:,1:20)))./sum(Y(1,:,:,:,:,:,1:20),4))...
%                 .*sum(Y(1,:,:,1:3,:,:,1:20),4);
