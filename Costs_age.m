...
if s > 1
    variance = 0.05;
    c_F012 = random(makedist('Triangular','a',c_F012*(1-variance),'b',c_F012,'c',c_F012*(1+variance)));
    c_F3 = random(makedist('Triangular','a',c_F3*(1-variance),'b',c_F3,'c',c_F3*(1+variance)));
    c_F4 = random(makedist('Triangular','a',c_F4*(1-variance),'b',c_F4,'c',c_F4*(1+variance)));
    c_DC = random(makedist('Triangular','a',c_DC*(1-variance),'b',c_DC,'c',c_DC*(1+variance)));
    c_HCC_pa = random(makedist('Triangular','a',c_HCC_pa*(1-variance),'b',c_HCC_pa,'c',c_HCC_pa*(1+variance)));
    c_NSP = random(makedist('Triangular','a',c_NSP*(1-variance),'b',c_NSP,'c',c_NSP*(1+variance)));
    c_OST = random(makedist('Triangular','a',c_OST*(1-variance),'b',c_OST,'c',c_OST*(1+variance)));
end
...
C1a=0;%trapz(TT,sum(sum(y(:,1,:,1:3),3),4).*exp(-discount.*TT))*c_svr_pa(1);
C1b=0;%trapz(TT,sum(y(:,1,:,4),3).*exp(-discount.*TT))*c_svr_pa(2);
C1c=0;%trapz(TT,sum(y(:,1,:,5),3).*exp(-discount.*TT))*c_svr_pa(3);
C2a=sum((sum(y(:,1,:,23),3)*(c_daa(1)+c_PWID)+sum(y(:,1,:,23),3)).*exp(-discount.*TT)); %Cost of treatment from the F0-F3 stages
C2b=sum((sum(y(:,1,:,25),3)*(c_daa(2)+c_PWID)+sum(y(:,1,:,25),3)).*exp(-discount.*TT)); %Cost of treatment from the F4 stage onwards
...
C1a=0;%trapz(TT,sum(sum(sum(y(:,2:3,:,1:3),2),3),4).*exp(-discount.*TT))*c_svr_pa(1);
C1b=0;%trapz(TT,sum(sum(y(:,2:3,:,4),2),3).*exp(-discount.*TT))*c_svr_pa(2);
C1c=0;%trapz(TT,sum(sum(y(:,2:3,:,5),2),3).*exp(-discount.*TT))*c_svr_pa(3);
C2aa=sum((sum(sum(y(:,2:3,:,23),2),3)*(c_daa(1)+c_formerPWID)+sum(sum(y(:,2:3,:,23),2),3)).*exp(-discount.*TT)); %Cost of treatment from the F0-F3 stages
C2bb=sum((sum(sum(y(:,2:3,:,25),2),3)*(c_daa(2)+c_formerPWID)+sum(sum(y(:,2:3,:,25),2),3)).*exp(-discount.*TT)); %Cost of treatment from the F4 stage onwards
...
c_formertot=(C1a+C1b+C1c+C2aa+C2bb+C3+C4+C5+C6+C7+C8+C9+C10+C_F4transfer+C_HCCtransfer+C_LT); 
...
%%treatments
c_treat_total = [c_HR, c_cascade, C2a+C2b+C2aa+C2bb];
c_treat_total(4) = c-sum(c_treat_total);
